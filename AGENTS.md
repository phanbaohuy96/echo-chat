...
Guidance for AI coding agents working in EchoChat.

## Project Overview

EchoChat is a demo chat monorepo created from the Flutter base structure template. It includes a Flutter client, Dart Frog backend, shared core package, data-source module, reusable UI/theme/media/navigation plugins, code generation tooling, localization generation, and distribution scaffolding.

**Product identity**: EchoChat
**Default locale**: English (`en`)
**Secondary locale**: Vietnamese (`vi`)
**Current technical app ID**: `com.pbh.baseflutter`

**Stack**: Flutter 3.41.7, Dart SDK `>=3.8.0 <4.0.0` for the backend, Kotlin 2.1.0, AGP 8.9.1, Gradle 8.12, Java 17, Dart Frog 1.2.x.
...
- **Networking**: Retrofit + Dio via `core` and `modules/data_source`
- **Local storage**: Hive, SharedPreferences, secure storage, SQLite-backed chat persistence
- **Routing**: project route abstractions under `presentation/route`
- **Localization**: CSV source files -> ARB -> generated localizations
- **Backend**: Dart Frog routes with services and in-memory demo storage

Dependency flow:

```text
apps/main -> core -> modules/data_source -> plugins
apps/backend -> Dart Frog routes/services/store
```

## Project Structure

```text
apps/main/           # Main Flutter app entry point, flavors, and EchoChat UI
apps/backend/        # Dart Frog backend for auth/chat demo APIs
core/                # Shared package: utilities, services, base widgets, app locale/theme helpers
modules/
  data_source/       # DTO, Hive, JSON, and repository plumbing shared by client code
plugins/
...
- `apps/backend/routes/health.dart` - health check
- `apps/backend/routes/api/auth/*` - signup, signin, current user
- `apps/backend/routes/api/chat/*` - chat users and messages
- `apps/backend/routes/_middleware.dart` - Dart Frog middleware
...
make pub_get        # Dependencies across plugins, core, main app, and backend
make pub_get_backend
...
make coverage_main
make test_backend   # Run Dart Frog backend tests
make run_backend    # Run the Dart Frog backend
make run_web_dev    # Run Flutter web dev flavor on port 3000
...
make gen_data_source  # build_runner + export generation in modules/data_source
...
- In app widgets, use `l10n.<key>` or `context.l10n.<key>`.
- In shared/core widgets, use `coreL10n` / `context.coreL10n`; in media widgets, use `flMediaL10n` / `context.flMediaL10n`.
...
## EchoChat API and Backend Rules

Backend business logic lives under `apps/backend/lib/src/services`; route handlers under `apps/backend/routes` should validate the request, call services, and return response helpers from `apps/backend/lib/src/http/responses.dart`.

Current backend model:

- `AuthService` handles signup, signin, bearer-token authentication, and token issuing.
- `ChatService` handles user listing, message listing, and sending.
- `DemoStore` is intentionally in-memory storage for the v1 demo.
- Backend tokens are for the demo API only; do not expose backend secrets to the Flutter client.

When changing API behavior:

- Keep Dart Frog route paths, request/response JSON, Flutter DTOs, Retrofit declarations, and usecases aligned.
- Put shared client-side DTOs in `modules/data_source`.
...
- Update local-first chat persistence deliberately so pending/sent/failed message behavior stays coherent.
- Run `make test_backend` after backend service or route changes.
...
make gen_data_source  # If data-source generated inputs changed
make gen_main         # If app generated inputs changed
make test_backend     # If backend behavior changed
...
For UI changes, run the app and smoke-test the affected flow before reporting completion whenever possible. For EchoChat, verify signup, signin, navigation to chat, and sending a chat message against the local backend. If you cannot run the UI in this environment, say so explicitly.
...
8. Keep Flutter client and Dart Frog backend contracts aligned.
9. Do not add speculative abstractions, fallback logic, or comments unless they are needed for the current task.
