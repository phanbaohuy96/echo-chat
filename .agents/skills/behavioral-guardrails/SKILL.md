...
description: Behavioral guidelines for EchoChat tasks: clarify ambiguity, keep changes simple and surgical, and define verifiable success criteria before coding.
license: MIT
compatibility: all
metadata:
...
Use this skill when writing, reviewing, refactoring, or planning code in EchoChat. It adapts the reusable ideas from the Karpathy-inspired `claude-skills` project to this repo's conventions.
...
- Ask before changing package IDs, bundle IDs, signing paths, CI/CD, generated workflows, backend API contracts, or broad formatting.
...
- Chat or auth API changes must keep the Flutter client DTOs, Retrofit declarations, and Dart Frog route contracts aligned.
- New client features should use `module-scaffold`, `bloc-pattern`, and `route-config` rather than ad-hoc Flutter structure.
...
- Keep Dart Frog route handlers thin; put backend business logic in services.
...
- `rg` for stale identifiers, locale codes, old keys, or API route names.
- `make lang` after localization CSV edits.
- `make gen_core`, `make gen_data_source`, `make gen_main`, or `make gen_all` only after relevant generated inputs change.
- `fvm flutter analyze --no-pub` from affected Flutter packages.
- `make test_backend` after backend service or route changes.
- UI smoke tests for user-facing flow changes when a device/browser is available.

## 5. EchoChat source-of-truth rules
...
| BLoC `_StateData` / JSON / DI | annotated Dart files | `make gen_core`, `make gen_data_source`, `make gen_main`, or `make gen_all` |
| Flutter API client | Retrofit declarations and DTOs in `core` / `modules/data_source` | `make gen_core` / `make gen_data_source` |
| Backend API behavior | Dart Frog routes plus services under `apps/backend/lib/src/services` | `make test_backend` |
...
- [ ] Flutter client and backend contracts kept aligned when APIs change.
- [ ] Stale references searched after rename/localization/API work.
